...
const PROJECT_SLIDES = [
  {
    ...
    description:
      "A prosthetic leg controlled by electromyographic (EMG) signals, enabling intuitive, real-time movement.",
    image: "/projects/transmission.png",
  },
  {
    title: "Smart Medication Manager",
    description:
      "A device designed to enhance quality of life by automating medication schedules.",
    image: "/projects/medication.png",
  },
  {
    title: "ALEX Arm",
    description:
      "An upper-body exoskeleton engineered to prevent repetitive strain injuries.",
...
const SOCIAL_LINKS = [
  {
    href: "https://www.linkedin.com/company/albertabionix/",
    label: "LinkedIn",
    path: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z",
  },
  {
    href: "https://www.instagram.com/albertabionix/",
    label: "Instagram",
  ...
  {
    href: "mailto:uabiomed@ualberta.ca",
    label: "Email",
...
const GlassCard = ({ children, className = "", style = GLASS_STYLES }) => (
  <Tilt
    ...
    transitionSpeed={400}
  >
    <div className={`rounded-3xl p-8 shadow-lg ${className}`} style={style}>
      {children}
    </div>
  </Tilt>
);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <button
          onClick={() => move(-1)}
          className="z-20 p-3 rounded-full bg-white/30 backdrop-blur-sm hover:bg-white/50 transition-all text-2xl"
          aria-label="Previous"
        >
          ‹
        </button>
      ...
      <div className="flex justify-center gap-2 mt-6">
        {slides.map((_, i) => (
          <button
            ...
            style={{
              backgroundColor:
                i === curr ? COLORS.accent : `${COLORS.tertiary}99`,
            }}
          />
        ))}
      </div>
  ...
  >
    <div className="flex flex-col items-center text-center w-full">
      ...
      >
        ...
        {role && (
          <div
            className="text-xs whitespace-pre-line opacity-70"
...
const Index = () => {
    ...
    () =>
      MISSION_IMAGES.map((img, i) => (
        <img
          ...
          src={img}
          alt=""
          className="w-full aspect-[7/5] object-cover rounded-2xl shadow-xl"
        />
      )),
    [],
  );
    ...
    () =>
      PROJECT_SLIDES.map((p, i) => (
        <GlassCard key={i} className="text-center py-6">
          ...
          <p className="text-lg mb-6 max-w-2xl mx-auto opacity-80">
            {p.description}
          </p>
          <img
            src={p.image}
            alt={p.title}
            className="w-full max-w-2xl mx-auto aspect-square object-cover rounded-2xl shadow-xl"
          />
        </GlassCard>
      )),
    [],
  ...
  return (
    ...
    >
      {/* Navigation */}
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all h-16 flex items-center ${isScrolled ? "shadow-md backdrop-blur-md border-b" : "bg-transparent"}`}
        style={{
          backgroundColor: isScrolled
            ? `${COLORS.background}e6`
            : "transparent",
          borderBottomColor: `${COLORS.tertiary}33`,
        }}
      >
          ...
          <div className="hidden md:flex items-center gap-8">
            {["mission", "projects", "people"].map((s) => (
              <button
                key={s}
                onClick={() => navScroll(s)}
                className="text-sm font-semibold capitalize hover:text-[#7e0000] transition-colors"
                style={{ color: isScrolled ? COLORS.primary : COLORS.white }}
              >
                {s}
              </button>
            ))}
            <Button
              ...
              className="hover:opacity-65 active:opacity-80 transition-opacity"
              asChild
            >
              <a href="https://forms.gle/SMaNMvi8qLGoNLtu6">Join Us</a>
            </Button>
          </div>
        </div>
      </nav>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 px-4">
          ...
          >
            <h1 className="text-5xl sm:text-8xl font-bold mb-6 text-white">
              Alberta Bionix
            </h1>
            <p className="text-xl sm:text-2xl font-light mb-10 text-white/90">
              A student-led engineering project group at the University of
              Alberta,
              <br />
              currently developing an EMG-controlled prosthetic leg.{" "}
            </p>
            <Button
              size="lg"
              style={{ backgroundColor: COLORS.accent }}
              className="px-8 py-6 text-lg mb-8 hover:opacity-65 active:opacity-80 transition-opacity"
              asChild
            >
              <a href="https://forms.gle/SMaNMvi8qLGoNLtu6">Join Us</a>
            </Button>
          ...
          <div className="flex flex-col gap-3 md:flex-row items-center">
            ...
            <div className="md:w-1/2 text-left">
              {[
                ...
                <div
                  key={i}
                  className="text-[clamp(1.7rem,4.1vw,3.5rem)] font-extralight"
                >
                  <span
                    className="font-bold"
                    style={{ color: COLORS.secondary }}
                  ...
                  <span style={{ color: COLORS.tertiary }}>{n}</span>
                </div>
              ))}
            </div>
      ...
      >
        ...
        <Button
          size="lg"
          style={{ backgroundColor: COLORS.accent }}
          className="px-8 py-6 text-lg mb-6 hover:opacity-65 active:opacity-80 transition-opacity"
          asChild
        >
          <a href="https://forms.gle/SMaNMvi8qLGoNLtu6">Join Us</a>
        </Button>
